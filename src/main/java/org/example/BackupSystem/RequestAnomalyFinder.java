...
public class RequestAnomalyFinder
{
    ...
    {
        Loggers.InitializeAnomalyPasswordRequestLogger();
        ReadAndCombineRequestLog();
    }

    public static ArrayList<String> ReadAndCombineRequestLog()
    {
        readLastPosition();

        ArrayList<String> loglist = new ArrayList<String>();
        String filePath = "src/main/java/org/example/Logs/PasswordRequest.txt";
        try (RandomAccessFile raf = new RandomAccessFile(filePath, "r"))
        {
            ...
            while ((line1 = raf.readLine()) != null)
            {
                ...
                if (line2 != null) {
                    ...
                    if (logEntry.contains("send password"))
                    {
    ...
    {
        return logline.substring(0, 22) + logline.split("INFO: User ")[1].split(" ")[0];
    }
    ...
    public static void FindAnomalies(ArrayList<String> logs) throws ParseException
    {
        ...
        for (Map.Entry<String, List<Date>> entry : userLogs.entrySet())
        {
            ...
            for (int i = 0; i < timestamps.size() - 2; i++)
            {
                ...
                {
                    Loggers.anomaly_password_request_logger.info("User " + userName + " send password change request 3 times in 30 seconds");
                    break;
                }
            }
        }
    ...
    {
        try (BufferedWriter writer = new BufferedWriter(new FileWriter("src/main/java/org/example/Anomalies/lastRequest.txt")))
        {
    ...
    {
        try (BufferedReader reader = new BufferedReader(new FileReader("src/main/java/org/example/Anomalies/lastRequest.txt")))
        {
